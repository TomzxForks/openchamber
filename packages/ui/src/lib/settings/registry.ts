...
export const SETTINGS_REGISTRY = {
  ...
  messageTimestampFormat: field({ scope: 'profile', parse: parseOneOf(['hidden', 'relative', 'absolute', 'hybrid']), ui: uiStore('messageTimestampFormat', (v) => useUIStore.getState().setMessageTimestampFormat(v)) }),
  messageTimestampHybridThresholdMinutes: field({ scope: 'profile', parse: parseIntegerAtLeast(1), ui: uiStore('messageTimestampHybridThresholdMinutes', (v) => useUIStore.getState().setMessageTimestampHybridThresholdMinutes(v)) }),
